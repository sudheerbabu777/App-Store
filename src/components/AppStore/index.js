...
class AppStore extends Component {
  ...
  getAppItemId = searchAppList => {
    const {activeTabId} = this.state
    const filterAppId = searchAppList.filter(
  ...
  render() {
    ...
    const searchResults = this.activeSearchResults()
    const filterAppId = this.getAppItemId(searchResults)
    ...
    return (
      <div className="app-container">
        ...
        <ul className="tab-list">
          {tabsList.map(each => (
            <TabItem
              tabsListDetails={each}
              key={each.tabId}
              clickTabButton={this.clickTabButton}
        ...
        <ul className="app-item-list">
          {filterAppId.map(each => (
            <AppItem appsListDetails={each} key={each.appId} />
          ))}
        </ul>
      </div>
    )
